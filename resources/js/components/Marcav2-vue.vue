  ...
  <div class='card-body'><h1 class=''>Lista de Marcav2 </h1>  
    ...
    <div class='col-12'>
      <b-button v-if="$can('Marcav2 Anadir')"  @click='anadir_registro()' type='button' class='btn btn-wangir btn-lg' data-toggle='button' size='sm' aria-pressed='false' variant='success' style='margin-bottom: 5px; margin: 5px;'>Añadir registro
      </b-button>
      <router-link v-if="$can('Marcav2 Anadir')" :to="{ name: 'marcav2formadd',params:{id:0} }">
      ...
      <template v-slot:cell(Acciones)='data'>
          ...
          <b-button-group>
          <!--  
          <b-button v-if="$can('Marcav2 Editar')" size='sm' variant='warning'  type='button' class='btn-sm btn btn-wangir mr-1' data-toggle='button' aria-pressed='false' style='margin-bottom: 5px; margin: 5px;'>Editar
            </b-button>
            -->
            <router-link :to="{ name: 'marcav2form', params: { id: data.item.id }}">
                <a v-bind:href="'/Marcav2/'+data.item.id+'/edit'" class='btn-sm btn btn-success mr-1' size='sm' style='margin-bottom: 5px; margin: 5px;'>
                    Editar 
                </a>
            </router-link>
            ...
            <b-button v-if="$can('Marcav2 Eliminar')"  v-b-modal.moda-eliminar @click='eliminar_registro(data.item.id)'
...
export default {
  ...
  methods: {
    ...
    consulta(page = 1){
      if (localStorage.getItem("Marcav2")) {
        try { this.items = JSON.parse(localStorage.getItem("Marcav2"));
        } catch (e) { localStorage.removeItem("Marcav2");}
      }

      this.page=page;
      //axios.get("Marcav2/consulta?page=" +page+"&consulta_data="+this.input_consulta_data)
      axios.get(`Marcav2/consulta?page=${page}&consulta_data=${this.input_consulta_data}`)
      .then(response => {
        this.consulta_datos = response.data;
        const parsed = JSON.stringify(response.data);
        localStorage.setItem("Marcav2", parsed);
      });
    },
    
    eliminar_registro(data_id){
    this.input_Marcav2_id=data_id;
    },
    eliminar_registro_delete(){
      var data_id=this.input_Marcav2_id;
      axios.delete(`Marcav2/${this.input_Marcav2_id}`).then(response => {
