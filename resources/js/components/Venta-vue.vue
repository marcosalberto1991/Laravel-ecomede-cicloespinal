  ...
  <div class='card-body'><h1 class=''>Lista de Venta </h1>
    ...
    <div class='col-12'>
        <!--
        <b-button v-if="$can('Venta Anadir')"  @click='anadir_registro()' type='button' class='btn btn-wangir btn-lg' data-toggle='button' size='sm' aria-pressed='false' variant='success' style='margin-bottom: 5px; margin: 5px;'>Añadir registro
        </b-button>
        -->
      <router-link v-if="$can('Venta Anadir')" :to="{ name: 'ventaformadd',params:{id:0} }">
      ...
      <template v-slot:cell(Acciones)='data'>
          ...
          <b-button-group>
          <!--
          <b-button v-if="$can('Venta Editar')" size='sm' variant='warning'  type='button' class='btn-sm btn btn-wangir mr-1' data-toggle='button' aria-pressed='false' style='margin-bottom: 5px; margin: 5px;'>Editar
            </b-button>
            -->
            <router-link :to="{ name: 'ventaform', params: { id: data.item.id }}">
                <a v-bind:href="'/Venta/'+data.item.id+'/edit'" class='btn-sm btn btn-success mr-1' size='sm' style='margin-bottom: 5px; margin: 5px;'>
                    Editar
                </a>
            </router-link>
            ...
            <b-button v-if="$can('Venta Eliminar')"  v-b-modal.moda-eliminar @click='eliminar_registro(data.item.id)'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      { key: 'estado_venta_id', sortable:true},
  ...
  methods: {
    ...
    consulta(page = 1){
      if (localStorage.getItem("Venta")) {
        try { this.items = JSON.parse(localStorage.getItem("Venta"));
        } catch (e) { localStorage.removeItem("Venta");}
      }

      this.page=page;
      //axios.get("Venta/consulta?page=" +page+"&consulta_data="+this.input_consulta_data)
      axios.get(`Venta/consulta?page=${page}&consulta_data=${this.input_consulta_data}`)
      .then(response => {
        this.consulta_datos = response.data;
        const parsed = JSON.stringify(response.data);
        localStorage.setItem("Venta", parsed);
      });
    },

    eliminar_registro(data_id){
    this.input_Venta_id=data_id;
    },
    data_foraneo(){

    },
    eliminar_registro_delete(){
      var data_id=this.input_Venta_id;
      axios.delete(`Venta/${this.input_Venta_id}`).then(response => {
