  ...
  <div class='card-body'><h1 class=''>Lista de Categoria </h1>
    ...
    <div class='col-12'>
        <!--  
        <b-button v-if="$can('Categoria Anadir')"  @click='anadir_registro()' type='button' class='btn btn-wangir btn-lg' data-toggle='button' size='sm' aria-pressed='false' variant='success' style='margin-bottom: 5px; margin: 5px;'>Añadir registro
        </b-button>
        -->
      <router-link v-if="$can('Categoria Anadir')" :to="{ name: 'categoriaformadd',params:{id:0} }">
      ...
      <template v-slot:cell(Acciones)='data'>
          ...
          <b-button-group>
          <!--
          <b-button v-if="$can('Categoria Editar')" size='sm' variant='warning'  type='button' class='btn-sm btn btn-wangir mr-1' data-toggle='button' aria-pressed='false' style='margin-bottom: 5px; margin: 5px;'>Editar
            </b-button>
            -->
            <router-link v-if="$can('Categoria Editar')" :to="{ name: 'categoriaform', params: { id: data.item.id }}">
                <a v-bind:href="'/Categoria/'+data.item.id+'/edit'" class='btn-sm btn btn-success mr-1' size='sm' style='margin-bottom: 5px; margin: 5px;'>
                    Editar
                </a>
            </router-link>
            ...
            <b-button v-if="$can('Categoria Eliminar')"  @click='eliminar_registro(data.item.id)'
  ...
  methods: {
    ...
    consulta(page = 1){
      if (localStorage.getItem("Categoria")) {
        try { this.items = JSON.parse(localStorage.getItem("Categoria"));
        } catch (e) { localStorage.removeItem("Categoria");}
      }

      this.page=page;
      //axios.get("Categoria/consulta?page=" +page+"&consulta_data="+this.input_consulta_data)
      axios.get(`Categoria/consulta?page=${page}&consulta_data=${this.input_consulta_data}`)
      .then(response => {
        this.consulta_datos = response.data;
        const parsed = JSON.stringify(response.data);
        localStorage.setItem("Categoria", parsed);
      });
    },

    eliminar_registro(data_id){
            this.input_Categoria_id=data_id;
    ...
    eliminar_registro_delete(){
      var data_id=this.input_Categoria_id;
      axios.delete(`Categoria/${this.input_Categoria_id}`).then(response => {
