  ...
  <div class='card-body'><h1 class=''>Lista de Estado_Venta </h1>
    ...
    <div class='col-12'>
        <!--
        <b-button v-if="$can('Estado_Venta Anadir')"  @click='anadir_registro()' type='button' class='btn btn-wangir btn-lg' data-toggle='button' size='sm' aria-pressed='false' variant='success' style='margin-bottom: 5px; margin: 5px;'>Añadir registro
        </b-button>
        -->
      <router-link v-if="$can('Estado_Venta Anadir')" :to="{ name: 'estado_ventaformadd',params:{id:0} }">
      ...
      <template v-slot:cell(Acciones)='data'>
          ...
          <b-button-group>
          <!--
          <b-button v-if="$can('Estado_Venta Editar')" size='sm' variant='warning'  type='button' class='btn-sm btn btn-wangir mr-1' data-toggle='button' aria-pressed='false' style='margin-bottom: 5px; margin: 5px;'>Editar
            </b-button>
            -->
            <router-link :to="{ name: 'estado_ventaform', params: { id: data.item.id }}">
                <a v-bind:href="'/Estado_Venta/'+data.item.id+'/edit'" class='btn-sm btn btn-success mr-1' size='sm' style='margin-bottom: 5px; margin: 5px;'>
                    Editar
                </a>
            </router-link>
            ...
            <b-button v-if="$can('Estado_Venta Eliminar')"  v-b-modal.moda-eliminar @click='eliminar_registro(data.item.id)'
...
export default {
  ...
  methods: {
    ...
    consulta(page = 1){
      if (localStorage.getItem("Estado_Venta")) {
        try { this.items = JSON.parse(localStorage.getItem("Estado_Venta"));
        } catch (e) { localStorage.removeItem("Estado_Venta");}
      }

      this.page=page;
      //axios.get("Estado_Venta/consulta?page=" +page+"&consulta_data="+this.input_consulta_data)
      axios.get(`Estado_Venta/consulta?page=${page}&consulta_data=${this.input_consulta_data}`)
      .then(response => {
        this.consulta_datos = response.data;
        const parsed = JSON.stringify(response.data);
        localStorage.setItem("Estado_Venta", parsed);
      });
    },

    eliminar_registro(data_id){
    this.input_Estado_Venta_id=data_id;
    },
    data_foraneo(){

    },
    eliminar_registro_delete(){
      var data_id=this.input_Estado_Venta_id;
      axios.delete(`Estado_Venta/${this.input_Estado_Venta_id}`).then(response => {
